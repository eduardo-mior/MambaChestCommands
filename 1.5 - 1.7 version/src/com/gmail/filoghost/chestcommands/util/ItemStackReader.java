...
public class ItemStackReader {
	...
	public ItemStackReader(String input, boolean parseAmount) throws FormatException {
		Validate.notNull(input, "o input (valor de entrada) nao pode ser nulo");
		...
		if (parseAmount) {
			...
			if (splitAmount.length > 1) {
				...
				if (!Utils.isValidInteger(splitAmount[1])) {
					throw new FormatException("quantia invalida \"" + splitAmount[1] + "\"");
				}
			
				int amount = Integer.parseInt(splitAmount[1]);
				if (amount <= 0) throw new FormatException("quantai invalida \"" + splitAmount[1] + "\"");
				this.amount = amount;
		...
		if (splitByColons.length > 1) {
			...
			if (!Utils.isValidShort(splitByColons[1])) {
				throw new FormatException("DataTag (value) invalida \"" + splitByColons[1] + "\"");
			}
		...
		if (material == null || material == Material.AIR) {
			throw new FormatException("Material invalido \"" + input + "\"");
		}
