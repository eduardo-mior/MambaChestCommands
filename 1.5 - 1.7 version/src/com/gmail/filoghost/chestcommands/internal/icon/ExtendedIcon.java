		...
		if (!player.hasPermission(Permissions.BYPASS_ECONOMY)) {
			...
			if (moneyPrice > 0) {
				if (!EconomyBridge.hasValidEconomy()) {
					player.sendMessage(ChatColor.RED + "Este comando tem um preço, mas o plugin de economia não pode ser encontrado ou não é compatível com o Vault. Por segurança, o comando foi bloqueado. Por favor informe este erro a um Staff!");
					return closeOnClick;
				}
			...
			if (playerPointsPrice > 0) {
				if (!PlayerPointsBridge.hasValidPlugin()) {
					player.sendMessage(ChatColor.RED + "Este comando tem um preço em pontos, mas o plugin PlayerPoints não foi encontrado. Por segurança, o comando foi bloqueado. Por favor informe este erro a um Staff!");
					return closeOnClick;
				}
			...
			if (moneyPrice > 0) {
				if (!EconomyBridge.takeMoney(player, moneyPrice)) {
					player.sendMessage(ChatColor.RED + "Erro: A transação não pode ser efetuada. Por favor informe este erro a um Staff!");
					return closeOnClick;
				}
