	...
	public void onSignChange(SignChangeEvent event) {
		if (event.getLine(0).equalsIgnoreCase("[menu]") && event.getPlayer().hasPermission(Permissions.SIGN_CREATE)) {
			...
			if (event.getLine(1).isEmpty()) {
				...
				event.getPlayer().sendMessage(ChatColor.RED + "Você deve informar o nome de um menu válido na segunda linha da placa.");
				return;
			}
			...
			if (iconMenu == null) {
				event.setLine(0, ChatColor.RED + event.getLine(0));
				event.getPlayer().sendMessage(ChatColor.RED + "O menu especificado não foi encontrado!");
				return;
			}
				
			event.setLine(0, ChatColor.DARK_BLUE + event.getLine(0));
			event.getPlayer().sendMessage(ChatColor.GREEN + "Placa criada com sucesso para o menu " + Utils.addYamlExtension(event.getLine(1)) + ".");
		}
	}
