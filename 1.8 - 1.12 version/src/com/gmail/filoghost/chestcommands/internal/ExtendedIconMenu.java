...
public class ExtendedIconMenu extends IconMenu {
	...
	public ExtendedIconMenu(String title, int rows, String type, String fileName) {
		...
		try {
			if (openActions != null) {
				for (IconCommand openAction : openActions) {
					openAction.execute(player, null);
				}
			}
			
			Inventory inventory;
			
			if (type.equals("CHEST")) inventory = Bukkit.createInventory(new MenuInventoryHolder(this), icons.length, title);
			else inventory = Bukkit.createInventory(new MenuInventoryHolder(this), InventoryType.valueOf(type), title);		
			...
			for (int i = 0; i < icons.length; i++) {
				if (icons[i] != null) {
					...
					inventory.setItem(i, icons[i].getHideAttributes() ? AttributeRemover.hideAttributes(icons[i].createItemstack(player)) : icons[i].createItemstack(player));
				}
			}
			...
			e.printStackTrace();
			player.sendMessage(ChatColor.RED + "Um erro interno ocorreu ao tentar abrir este menu. Por favor informe este erro a um Staff!");
		}
	}
	...
	public void refresh(Player player, Inventory inventory) {
		try {
			...
			e.printStackTrace();
			player.sendMessage(ChatColor.RED + "Um erro interno ocorreu ao tentar atualizar este menu. Por favor informe este erro a um Staff!");
		}
	}
