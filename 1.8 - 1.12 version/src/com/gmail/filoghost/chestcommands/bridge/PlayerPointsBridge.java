...
public class PlayerPointsBridge {
	...
	public static int getPoints(Player player) {
		if (!hasValidPlugin()) throw new IllegalStateException("O plugin PlayerPoints nao foi encontrado!");
		return playerPoints.getAPI().look(player.getUniqueId());
	}
	
	public static boolean hasPoints(Player player, int minimum) {
		if (!hasValidPlugin()) throw new IllegalStateException("O plugin PlayerPoints nao foi encontrado!");
		if (minimum < 0) throw new IllegalArgumentException("Quantia de pontos invalida: " + minimum);
