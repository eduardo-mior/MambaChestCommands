...
public class Icon {
	...
	public void setHideAtributes (boolean hideAttributes) {
	...
	protected List<String> calculateLore(Player pov) {
		...
		if (material == null) {
			...
			output.add(ChatColor.RED + "(Material invalido)");
		}
	...
	public ItemStack createItemstack(Player pov) {
		...
		if (skullUrl != null && itemMeta instanceof SkullMeta) {
			...
			try {
				profileField.set(((SkullMeta) itemMeta), profile);
	...
	public boolean onClick(Player whoClicked, ClickType click) {
		if (clickHandler != null) {
			return clickHandler.onClick(whoClicked, click);
		}
